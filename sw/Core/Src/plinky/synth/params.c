...
static s8 value_to_index(s32 value, u8 range) {
	return clampi(value, -65535, 65535) * range >> 16;
}
...
void save_param_raw(Param param_id, ModSource mod_src, s16 data) {
	...
	case P_TEMPO:
		...
		// I'm leaving it like this until there is a better way to save values with larger ranges, with lower limits
		if (mod_src == SRC_BASE)
			data = maxi(data, -768);
		break;
	case P_LATCH_TGL:
...
void select_mod_src(ModSource mod_src) {
	if (selected_param == P_VOLUME) {
		flash_message(F_20_BOLD, I_CROSS "No Modulation", "");
		return;
	}
...
static const char* get_param_str(Param param_id, ModSource mod_src, s16 raw, char* val_buf) {
	...
	case P_Y_SCALE:
		disp_range_10x = 2000;
		break;
	// show negative value as positive
	case P_SWING:
	case P_ARP_CHANCE:
	case P_SEQ_CHANCE:
		raw = abs(raw);
	...
	case P_PING_PONG:
		...
		break;
	case P_TEMPO:
		disp_val_10x = clock_type == CLK_INTERNAL ? (raw + RAW_SIZE) * 1200 >> 10 : bpm_10x;
		break;
	case P_VOLUME:
...
void draw_cur_param(void) {
	...
	if (base_raw < 0) {
		...
		case P_SEQ_CHANCE:
			...
			draw_str(text_x, 20, F_12_BOLD, "Chance (H)");
			return;
		default:
			break;
		}
